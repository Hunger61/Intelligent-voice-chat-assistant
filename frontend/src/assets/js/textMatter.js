...
export function addNewObject(text,i) {
  ...
  const initialY = Math.random() * (containerHeight * 0.5) + containerHeight * 0.2; // 屏幕上方区域
  ...
  // 设置初始速度，形成抛物线
  const horizontalSpeed = isLeft ? 5 + Math.random() * 3 : -(5 + Math.random() * 3); // 水平速度
  const verticalSpeed = -3 - Math.random() * 2; // 初始向上的垂直速度，结合重力形成抛物线
