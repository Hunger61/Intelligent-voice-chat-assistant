<template>
  <div id="app">
    <!-- 全局加载动画（作为过渡动画） -->
    <div v-if="isLoading" class="loading-overlay" :style="{ opacity: loadingOverlayOpacity }">
      <div class="svg-container">
        <svg viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg">
          ...
          <path class="path fill"
            ...
            p-id="2163" @animationend="onLoadingAnimationComplete" />
        </svg>
      </div>
    </div>

    <!-- 路由视图（移除原过渡动画，仅保留组件渲染） -->
    <router-view v-slot="{ Component }">
      <component :is="Component" class="page" @page-loaded="handlePageLoaded" :style="{ opacity: pageOpacity }" />
    </router-view>
  </div>
</template>

<script setup>
import {  ref, watch } from 'vue'; // 导入Composition API相关函数
import { useStore } from 'vuex'; // 导入useStore
import { useRoute } from 'vue-router' //路由依赖
...
const loadingOverlayOpacity = ref(1);
...
const resetLoadingState = () => {
  ...
  loadingOverlayOpacity.value = 1;
  pageOpacity.value = 0;
  loadingAnimationComplete.value = false;
  pageLoaded.value = false;
};
// 处理页面加载完成事件
const handlePageLoaded = () => {
  pageLoaded.value = true;
  if (loadingAnimationComplete.value) {
    startHideLoading();
  }
};
// 加载动画播放完成回调
