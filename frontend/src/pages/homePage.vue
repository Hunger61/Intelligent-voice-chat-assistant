      ...
      <div v-show="showModal" id="cover"
        ...
        <div v-if="showA"
          ...
          <div class="p-5">
            ...
            <!-- 预设助手选择 -->
            <div class="mb-5">
              <label class="block text-sm text-gray-600 mb-1.5">预设助手</label>
              <select v-model="selectedPreset"
                ...
                <option v-for="(preset, index) in presetAssistants" :key="index" :value="index">
                  {{ preset.name }}
                </option>
              </select>
            </div>
...
import { presetAssistants } from '../assets/js/move.js';
...
const voiceCharacters = [
  ...
  },
  {
    ...
    type: "情感女声",
    tags: [
      { text: "温柔", bg: "bg-pink-100", textColor: "text-pink-800" },
      { text: "共情", bg: "bg-indigo-100", textColor: "text-indigo-800" }
    ],
    isActive: true
  },
  {
    ...
    type: "新闻男声",
    tags: [
      { text: "沉稳", bg: "bg-gray-100", textColor: "text-gray-800" },
      { text: "清晰", bg: "bg-blue-100", textColor: "text-blue-800" }
    ],
    isActive: false
  },
  {
    ...
    type: "新闻女声",
    tags: [
      { text: "端庄", bg: "bg-purple-100", textColor: "text-purple-800" },
      { text: "流利", bg: "bg-green-100", textColor: "text-green-800" }
    ],
    isActive: false
  }
];//音色列表
...
watch(selectedPreset, (index) => {
  if (index >= 0 && index < presetAssistants.length) {
    const preset = presetAssistants[index];
    newAssistant.value.name = preset.name;
    newAssistant.value.description = preset.description;
    newAssistant.value.character = preset.character;
    newAssistant.value.knowledge_base_id = preset.knowledge_base_id || '';
  }
});
...
const addAssistant = async () => {
  ...
  try {
    ...
    const assistantId = typeof newId === 'string' ? newId : '';
    assistants.push({
      id: assistantId,
      name: newAssistant.value.name.trim(),
      description: newAssistant.value.description?.trim() || '',
    ...
    if (assistantId) {
      store.commit('setSelectedId', assistantId);
    }
...
const changeAssistant = async () => {
  ...
  isAdding.value = true;
  try {
    await AssistantService.modifyCommon(
      newAssistant.value.id,
      newAssistant.value.name.trim(),
      newAssistant.value.description?.trim() || '',
      newAssistant.value.character?.trim() || '你是一个AI语音助手',
      newAssistant.value.knowledge_base_id || ''
    );

    const assistants = store.getters.getAllAssistants;
    const index = assistants.findIndex(a => a.id === newAssistant.value.id);
    if (index !== -1) {
      assistants[index] = {
        ...assistants[index],
        name: newAssistant.value.name.trim(),
...
const removeAssistant = async (id) => {
  ...
  try {
    ...
    const assistants = store.getters.getAllAssistants.filter(a => a.id !== id);
    store.commit('setAssistants', [...assistants]);
    localAssistants.value = assistants;

    if (store.getters.getSelectedId === id) {
      store.commit('setSelectedId', assistants[0]?.id || '');
    }

    showSuccessMessage('删除助手成功');
  } catch (error) {
