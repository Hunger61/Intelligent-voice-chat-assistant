...
public class UserApplicationService {
    ...
    public String registerUser(String nickname, String password, String email, String verificationCode) {
        ...
        String storedCode = (String) redisUtil.get(RedisKeys.verificationCodeKey(userEmail));
        if (storedCode == null || !storedCode.equals(verificationCode)) {
            throw new BaseException(ErrorEnum.VERIFICATION_CODE_INCORRECT);
        }//todo 将storedCode直接判断是否存在
        redisUtil.delete(RedisKeys.verificationCodeKey(userEmail));
    ...
    public String login(String email, String password) {
        UserEmail userEmail = new UserEmail(email);
        User user = userDomainService.authenticate(userEmail, password);
        String token = jwtUtil.createToken(user.getId().toString());
        redisUtil.set(RedisKeys.userKey(user.getId()), "", 86400);//todo
        return token;
    }
    ...
    public void sendVerificationCode(String email) {
        ...
        redisUtil.set(RedisKeys.verificationCodeKey(userEmail), verificationCode, 300);
    ...
    public void logout(User user) {
        redisUtil.delete(RedisKeys.userKey(user.getId()));//todo
