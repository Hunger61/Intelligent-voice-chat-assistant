...
public class AIAssistantApplicationService {
    ...
    public String answerQuestion(String question, String aiAssistantId) {
        ...
        Dialogue byAIAssistantId = dialogueRepository.findByAIAssistantId(assistantId)// todo
                                                                                      // 对于dialogue考虑从redis等缓存中读取，减少数据库压力
                .orElseThrow(() -> new BaseException(ErrorEnum.DIALOGUE_NOT_FOUND));
        byAIAssistantId
                .addContext(new DialogueContext(new DialogueRole(DialogueRoles.USER), new DialogueContent(question)));
        ...
        byAIAssistantId.addContext(
                new DialogueContext(new DialogueRole(DialogueRoles.ASSISTANT), new DialogueContent(answer)));
    ...
    public void streamingAnswerQuestionAsync(String question, String aiAssistantId, Consumer<String> onToken,
        ...
        CompletableFuture.runAsync(() -> {
            try {
                ...
                // add user context locally and persist immediately so the downstream LLM
                // service can read saved context if it fetches from DB
                ...
                questionAnsweringService.streamingAnswerQuestionAsync(
                        ...
                        full -> {
                            try {
                                ...
                                if (fresh == null) {
                                    log.warn(
                                            "Dialogue disappeared when persisting full answer, creating new dialogue record");
